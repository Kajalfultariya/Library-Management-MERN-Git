...
const StudentLibraryManagement = () => {
  ...
  const fetchBookData = async () => {
    await axios.get("http://localhost:2000/api/fetch").then((response) => {
  ...
  const fetchStudentData = async () => {
    await axios.get("http://localhost:2000/api/fetchStudentOne/" + localStorage.getItem("studentId"))
      .then((response) => {
  ...
  const fetchBorrowBookData = async () => {
    await axios.get("http://localhost:2000/api/fetchBook").then((response) => {
      if (response.data)
  ...
  const fetchStudentListData = async () => {
    await axios.get("http://localhost:2000/api/fetchStudent").then((response) => {
      if (response.data)
    ...
    books.map(async (book) => {
      if (book.id === id) {
        if (!book.available.includes(JSON.parse(localStorage.getItem("studentPersonalId")))) {
          ...
          //update book list
          await axios.put(`http://localhost:2000/api/update/${bookId}`, formData)
            .then((response) => { }).catch(error => { console.log("errr", error) })


          //add boorow book data
          await axios.post("http://localhost:2000/api/createBook", newBorrow)
            .then((response) => {
            }).catch(error => { console.log("errr", error) })
          ...
          await axios.put(`http://localhost:2000/api/updateStudent/${studentInfo._id}`, formDataId)
            .then((response) => {
  ...
  const handleReturnBook = async (bookId, id) => {
    ...
    books.map(async (book) => {
      if (book.id === id) {
        ...
        //update book list
        await axios.put(`http://localhost:2000/api/update/${book._id}`, formData)
          .then((response) => {
        ...
        borrowedBooks.map(async (item) => (
          ...
          item.bookId === id && item.studentId === localStorage.getItem("studentId") ?
            await axios.delete(`http://localhost:2000/api/deleteBook/${item._id}`)
              .then((res) => {
